// app/ui/evaluate/evaluate-alphabet.js
"use client";
import LabelsAlphabet from "@/app/ui/evaluate/labels-alphabet";
...
const EvaluateAlphabet: React.FC = () => {
  const [selectedSearch, setSelectedSearch] = useState<{ letter: string; type: string } | null>(null); 
  ...
  const fetchSpecificSign = useCallback(async (letter: string, type: string) => {
    if (!letter.trim() || !type.trim()) {
      setSelectedSign(null);
      setErrorMessage('Por favor, selecciona una letra.');
      return;
    }
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        setErrorMessage(`No se encontró una seña activa de tipo "${type}" para la letra "${letter}".`);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (selectedSearch) {
      fetchSpecificSign(selectedSearch.letter, selectedSearch.type);
    }
  }, [selectedSearch, fetchSpecificSign]);

  const handleLetterAndTypeSelect = (letter: string, type: string) => {
    setSelectedSearch({ letter, type });
  };
  ...
  const handleRecordedVideoData = async (docId: string, videoDownloadUrl: string) => {
    ...
    try {
      ...
      // Ejemplo con fetch:
      const response = await fetch('https://model-lsc-api-244204625992.us-central1.run.app/predict_recognition_video_alphabet', { // Reemplaza con la URL de tu API
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // 'Authorization': `Bearer ${yourAuthToken}` // Si tu API requiere autenticación
        },
      ...
      try {
        ...
        await addDoc(evaluatesSignCollection, {
          ...
          model:'alphabet_1', // El modelo que estás usando
  ...
  return (
    ...
    <div className="p-4">
      ...
      <div className="mb-8">
        <h2 className="text-xl text-gray-800 mb-4">Selecciona una letra, luego el sistema te entregara las opciones de señas disponibles para evaluar y ser parte del conjunto de datos de prueba y evaluación:</h2>
        <LabelsAlphabet 
          onLetterSelect={handleLetterAndTypeSelect} 
          selectedLetter={selectedSearch?.letter || null} 
          signType="Caracter"           
        />
      </div>
      ...
      {selectedSign && userId && levelId && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
          <div className="bg-white shadow-lg rounded-lg p-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Grabar seña para evaluar: {selectedSign.name.toUpperCase()} </h2>
          ...
          <div className="bg-white shadow-lg rounded-lg p-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Video de Ejemplo: {selectedSign.name.toUpperCase()}</h2>
            <ExampleVideo
              name={selectedSign.name}
...
export default EvaluateAlphabet;