...
const TEXT_TO_SIGN_ENDPOINT = `${COLSIGN_API_BASE}/text-to-sign`;

// Marcador que devuelve la API cuando detecta groserías
const PROFANITY_MARKER = '...';
...
// Respuesta del endpoint /text-to-sign
interface TextToSignResponse {
  text_original: string;
  señas_seleccionadas: string[];
}

const VisualizeSignsInterpreteComponent: React.FC = () => {
  ...
  useEffect(() => {
    // Esperar a que termine la carga para que el elemento <video> esté montado
    if (isLoading) return;
  ...
  }, [currentVideoIndex, videos, handleVideoEnded, isLoading]); // Se ejecuta cuando el índice, la lista de videos o el estado de carga cambia
  ...
  // Construye la lista de videos a partir de las señas seleccionadas por la API.
  // Para cada seña intenta encontrar el video exacto; si no existe, deletrea por letras.
  const buildVideoResults = async (signs: string[]): Promise<VideoResult[]> => {
    ...
    for (let i = 0; i < signs.length; i++) {
      ...
      const exactSign = await fetchSign(sign);

      if (exactSign) {
        allVideoResults.push({ ...exactSign, originalInput: sign });
      } else {
        // No se encontró la seña completa: deletreamos letra por letra
      ...
      // Pausa entre señas (no después de la última)
      if (i < signs.length - 1) {
        allVideoResults.push({ name: 'Espacio', url: '', type: 'Separator', originalInput: ' ' });
      }
    }
  ...
  const saveTextToSignQuery = async (
    ...
    try {
      ...
      await addDoc(textToSignCollection, {
        ...
        had_profanity: hadProfanity,
        total_senas: selectedSigns.length,
        createdAt: new Date(),
      });
  ...
  const handleSearch = async () => {
    ...
    try {
      // 1. Enviar el texto a la API para obtener las señas seleccionadas
      const response = await fetch(TEXT_TO_SIGN_ENDPOINT, {
        method: 'POST',
      ...
      if (!response.ok) {
        let detail = 'Error al procesar el texto en la API.';
        try {
      ...
      const data: TextToSignResponse = await response.json();
      const rawSigns = Array.isArray(data?.señas_seleccionadas) ? data.señas_seleccionadas : [];

      // 2. Omitir el marcador de groserías ("...") para evitar errores y contenido ofensivo
      const hadProfanity = rawSigns.some((s) => s?.trim() === PROFANITY_MARKER);
      const filteredSigns = rawSigns
        .map((s) => (s ?? '').trim())
        .filter((s) => s.length > 0 && s !== PROFANITY_MARKER);

      if (filteredSigns.length === 0) {
        setError(
          hadProfanity
            ? 'El mensaje contiene lenguaje no permitido. Por favor, reformúlalo.'
            : 'No se generaron señas para tu mensaje. Intenta con otro texto.'
        );
      ...
      // 3. Construir la reproducción de videos a partir de las señas seleccionadas
      ...
      setError(
        err instanceof Error
          ? `Error al generar señas: ${err.message}`
          : 'Error al generar señas. Intenta nuevamente.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <main className="p-4 md:p-8 bg-gray-50 rounded-lg shadow-xl max-w-4xl mx-auto my-8">
      <h1 className="text-3xl font-extrabold text-blue-500 mb-6 text-center">Generador de Señas</h1>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <textarea
          ...
          placeholder="Ingresa tu mensaje (máximo 100 caracteres). Ejemplo: Hola quiero 2 fotocopias"
          rows={4}
        ...
        {videos.length > 0 && !isLoading && (
          <div className="w-full max-w-xl bg-white border rounded-lg shadow-lg p-4 mt-8">
            <h2 className="text-xl font-semibold text-gray-800 mb-4 text-center">
              Reproducción de Señas: {currentVideoToDisplay?.type !== 'Separator' ? currentVideoToDisplay?.name : ''}
...
export default VisualizeSignsInterpreteComponent;